--------------------------------------------------------------------------------
-- Wagon split, formerly known as "wagon control"
--------------------------------------------------------------------------------
...
cs2.register_combinator_mode({
	name = "wagon_split",
	localized_string = "cybersyn2-combinator-modes.wagon-split",
	settings_element = "CombinatorGui.Mode.WagonSplit",
	help_element = "CombinatorGui.Mode.WagonSplit.Help",
	is_output = true,
})
...
relm.define_element({
	name = "CombinatorGui.Mode.WagonSplit.Help",
	render = function(props)
		return VF({
			ultros.RtMultilineLabel({ "cybersyn2-combinator-mode-wagon-split.desc" }),
			Pr({
				type = "label",
			...
			Pr({
				...
				ultros.RtMultilineLabel({
					"cybersyn2-combinator-mode-wagon-split.output-signals",
				}),
			}),
		})
	end,
...
local function set_requester_wagon_combs(cstrain, stop, delivery)
	...
	if not manifests then
		stlib.error(
			"Wagon manifests were not precomputed on train arrival at requester..."
		)
		return
	end
	for _, comb in pairs(combs) do
		...
		if wagon then
			local index = get_wagon_index(cstrain, wagon)
			if index and manifests[index] then
				comb:direct_write_outputs(manifests[index])
			end
		end
	end
end
...
events.bind(
	...
	function(combinator, key, new_value, old_value)
		if
			...
			or (
				key == "mode"
				and (new_value == "wagon_split" or old_value == "wagon_split")
			)
		then
...
cs2.on_train_departed(function(train, cstrain, stop)
	...
	-- Clear output of wagon split combinators
	for _, comb in cs2.iterate_combinators(stop) do
		if comb.mode == "wagon_split" then comb:direct_write_outputs(empty) end
	end
...
events.bind("cs2.train_pre_arrived", function(train, cstrain, stop)
	...
	elseif delivery.to_id == stop.id then
		-- Precompute cargo values before arrival at requesters, because on the
		-- arrival tick, 1 tick of stuff has already been pulled out of the train's
		-- inventory by unloaders, which cause manifest signal inaccuracy.
		local manifests = {}
		for index, carriage in pairs(train.carriages) do
			manifests[index] = create_carriage_signals(carriage)
		end
		---@diagnostic disable-next-line: inject-field
		delivery.requester_wagon_manifests = manifests
	end
end)
