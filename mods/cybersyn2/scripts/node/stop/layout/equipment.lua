...
function _G.cs2.stop_api.register_loading_equipment(
	stop_id,
	entity,
	pos,
	is_cargo,
	...
	if not stop_state or not layout then
		return false
	end
	...
	if is_cargo then
		if previous_cargo ~= tile_index then
			changed = true
		end
		layout.cargo_loader_map[equipment_id] = tile_index
	else
		if previous_cargo then
			changed = true
		end
	...
	if is_fluid then
		if previous_fluid ~= tile_index then
			changed = true
		end
	...
	if changed and not scan_is_ongoing then
		cs2.raise_train_stop_equipment_changed(stop_state, layout)
	end
...
local function register_equipment_if_applicable(
	...
	if (not rail_bbox) or not stop_bbox then
		return
	end
	...
	if equipment.type == "inserter" then
		if bbox_contains(rail_bbox, equipment.pickup_position) then
			stop_api.register_loading_equipment(
				stop.id,
				equipment,
...
cs2.on_train_stop_layout_changed(function(stop, layout)
	scan_equipment(stop)
end)

-- When equipment entities are built or destroyed, update individually
cs2.on_built_equipment(function(equipment)
	built_or_destroyed_equipment(equipment, false)
end)

cs2.on_broken_equipment(function(equipment)
