...
function delivery_api.create_delivery(
		vehicle_id,
		manifest,
		source_id,
		destination_id,
	...
	raise_delivery_created(storage.deliveries[id])
	return storage.deliveries[id]
end

---Destroy a delivery.
---@param id Id
function delivery_api.destroy_delivery(id)
	local delivery = storage.deliveries[id]
	if not delivery then return end
	raise_delivery_destroyed(delivery)
...
function delivery_api.set_state(delivery, new_state)
	...
	if new_state == DeliveryState.Initializing then
		log.warn("Attempt to return delivery to Initializing state, id", delivery.id)
		return
	end
	-- Taking a delivery out of completed state is forbidden
	if old_state == DeliveryState.Completed or old_state == DeliveryState.Failed then
	...
	raise_delivery_state_changed(delivery, new_state, old_state)
